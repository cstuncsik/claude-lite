...
pub async fn create_message(pool: &SqlitePool, message: Message) -> Result<()> {
    ...
    sqlx::query(
        "INSERT INTO messages (id, chat_id, role, content, images, created_at) VALUES (?, ?, ?, ?, ?, ?)",
    )
    .bind(&message.id)
...
pub async fn list_messages(pool: &SqlitePool, chat_id: &str) -> Result<Vec<Message>> {
    let messages = sqlx::query_as::<_, Message>(
        "SELECT id, chat_id, role, content, images, created_at FROM messages WHERE chat_id = ? ORDER BY created_at ASC",
    )
    .bind(chat_id)
    .fetch_all(pool)
    .await?;
